var webEx = {
    ...
    ready(selector, callback, context=document) {
        ...
        , observerCallback = (records) => {
            for (let record of records) {
                ...
                let parent = record.target.parentNode || context;
    ...
    Navigate: {
        ...
        pushHistoryState(state, url = '', origin = document.location.origin, params = {}) {
            var url = new URL(url || window.location.href, origin);
    ...
    Form: function (form, options={}) {
        ...
        this.setResponseError = ({ id, msg }) => {
            if (this.form.elements[id]) {
                let element = new webEx.FormField(this.form.elements[id]);
                let resetError = () => {
                    element.resetError(); 
                };

                webEx.Events.remove('input change', element.element, resetError);
                webEx.Events.add('input change', element.element, resetError);

                if (!this.form.errors.length) {
                    webEx.Events.trigger('focus', element.element);
                }
        ...
        this.resetErrors = () => {
            for (let name in this.form.errors) {
                if (this.form.elements[name]) {
                    var element = new webEx.FormField(this.form.elements[name]);
                    element.resetError();
                }
            }
        };
        ...
        this.submit = () => {
            ...
            const promise = webEx.sendRequest(
                this.options.action, 
                this.formData, 
                this.options
            )
    ...
    CharCounter: function (el, options = {}) {
        ...
        this.init = () => {
            ...
            // console.log(style.display, style.visibility, this.element);
            ...
            this.counterElement = webEx.DOM.createElement(this.options.template);
            ...
            webEx.Events.add('input', this.element, function () {
                that.setCount();
            });
        ...
        this.setCount = () => {
            ...
            this.counterElement.innerHTML = webEx.setPlaceholders(this.options.format, {
                count,
    ...
    InputTags: function (el, options = {}) {
        ...
        this.options = {
            ...
            template: `<div class="input-tags" style=""></div>`,
            ...options
        };
        ...
        this.init = () => {
            ...
            this.inputElement = webEx.DOM.createElement(this.options.tplInput);
            this.containerElement = webEx.DOM.createElement(this.options.template);
            ...
            webEx.Events.add('input', this.inputElement, function (evt) {
            ...
            webEx.Events.add('keydown', this.inputElement, function(evt) {
        ...
        this.appendTag = (txt) => {
            ...
            let tagElement = webEx.DOM.createElement(this.options.tplTag);
            tagElement.dataset.idx = this.tags.length;
            tagElement.querySelector('.input-tag-text').innerText = txt;

            webEx.Events.add('click', tagElement.querySelector('.input-tag-remove'), function () {
                tagElement.remove();
                that.removeTag(tagElement.dataset.idx);
                webEx.Events.trigger('focus', that.inputElement);
            }, false);
        ...
        this.updateValue = () => {
            ...
            webEx.Events.trigger('change', this.element);
        };
    ...
    Modal: function (options = {}) {
        ...
        this.getElement = () => {
            if (this.options.id) {
                ...
                this.element = webEx.DOM.createElement(this.options.template, attributes);
        ...
        this.handleRequest = async (request, target) => {
            ...
            if (request instanceof Promise) {
                ...
                response = await webEx.sendRequest(
                    request.action
                    , request.data
                    , options
                );
        ...
        this.addButtons = (buttons) => {
            for (let btn of buttons) {
                ...
                let elem = document.createElement('button'); webEx.DOM.createElement(`<button></button>`, attrs);
                webEx.DOM.setAttrs(elem, attrs);

                if (btn.onClick) {
                    webEx.Events.add('click', elem, btn.onClick);
                }
    ...
    Preloader: function (context, options={}) {
        ...
        this.init = () => {
            ...
            this.preloader = webEx.DOM.createElement(this.options.template);
            webEx.DOM.setHTML(this.preloader, this.options.dialogTemplate);
            ...
            if (this.options.spinner === true) {
                ...
                webEx.DOM.setHTML(this.preloader.querySelector('.preloader-dialog'), this.options.spinnerTemplate);
    ...
    BtnLoading: function(context, options) {
        ...
        this.hide = () => {
            ...
            webEx.DOM.setHTML(this.context, this.originalContent);
        };
    ...
    sendRequest: function (action, data = {}, options = {}) {
        ...
        if (defaultOptions.preloader !== false) {
            ...
            if (contexts.length > 0) {
                for (let context of contexts) {
                    let instance = new webEx[preloader.options.instance](context, preloader.options);
        ...
        return axios({...defaultOptions, data})
          .then(response => {
            ...
            webEx.Request.handlers[response.statusText == 'OK' ? 'success' : 'failure'](response);
            webEx.Request.handlers.always(response);
    ...
    async loadContent(target, data, options={}) {
        const action = options.action || 'chunk/get';
        const contexts = document.querySelectorAll(target);
        
        if (options.clearTargetBeforeSend) {
