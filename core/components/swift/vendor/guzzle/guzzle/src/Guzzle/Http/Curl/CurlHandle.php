...
class CurlHandle
{
     ...
     * @return bool
     */
    public function isAvailable($handle = null)
    {
        $handle = $handle ? $handle : $this->handle;
        return is_a($handle, 'CurlHandle');
    }
    ...
    {
        if (!$this->isAvailable($this->handle)) {
            return null;
        }
