...
if (!empty($includeThumbs)) {
    ...
    if (!empty($thumbs[0])) {
        ...
        $miniShop2->pdoTools->addTime('Included list of thumbnails: <b>' . implode(', ', $thumbs) . '</b>.');
    }
}
...
$miniShop2->pdoTools->setConfig(array_merge($default, $scriptProperties), true);
...
$output = $miniShop2->pdoTools->getChunk($tpl, $pls);

if ($modx->user->hasSessionContext('mgr') && !empty($showLog)) {
    $output .= '<pre class="msGetOrderLog">' . print_r($miniShop2->pdoTools->getTime(), true) . '</pre>';
}
