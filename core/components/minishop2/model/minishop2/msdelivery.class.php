 ...
 * @property int rank
 */
class msDelivery extends xPDO\Om\xPDOSimpleObject
{
    ...
    */
    public function __construct(xPDO\xPDO $xpdo)
    {
        ...
        $this->ms2 = $this->xpdo->services->get('minishop2');
    }
    ...
    */
    public function loadHandler()
    {
        ...
        if (!class_exists($class)) {
            $this->xpdo->log(MODX\Revolution\modX::LOG_LEVEL_ERROR, 'Delivery handler class "' . $class . '" not found.');
            $class = 'msDeliveryHandler';
        }

        $this->handler = new $class($this, []);
        if (!($this->handler instanceof msDeliveryInterface)) {
            $this->xpdo->log(MODX\Revolution\modX::LOG_LEVEL_ERROR, 'Could not initialize delivery handler class: "' . $class . '"');
