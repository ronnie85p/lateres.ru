...
class msCustomerProfile extends xPDO\Om\xPDOObject
{
    ...
    */
    public static function load(xPDO\xPDO &$xpdo, $className, $criteria, $cacheFlag = true)
    {
        ...
        if (!is_object($instance) || !($instance instanceof $className)) {
            if (is_numeric($criteria) || (is_array($criteria) && !empty($criteria['id']))) {
                ...
                if ($xpdo->getCount('modUser', ['id' => $id])) {
