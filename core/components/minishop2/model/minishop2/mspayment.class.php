 ...
 * @property int rank
 */
class msPayment extends xPDO\Om\xPDOSimpleObject
{
    ...
    */
    public function loadHandler()
    {
        ...
        if (!class_exists($class)) {
            $this->xpdo->log(MODX\Revolution\modX::LOG_LEVEL_ERROR, 'Payment handler class "' . $class . '" not found.');
            $class = 'msPaymentHandler';
        }

        $this->handler = new $class($this, []);
        if (!($this->handler instanceof msPaymentInterface)) {
            $this->xpdo->log(MODX\Revolution\modX::LOG_LEVEL_ERROR, 'Could not initialize payment handler class: "' . $class . '"');
