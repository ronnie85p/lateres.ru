...
class msProductFile extends xPDOSimpleObject
{
     ...
     * @return bool|string
     */
    public function prepareSource(MODX\Revolution\sources\modMediaSource $mediaSource = null)
    {
        $this->miniShop2 = $this->xpdo->services->get('minishop2');
        if ($mediaSource) {
            $this->mediaSource = $mediaSource;

            return true;
        } elseif (is_object($this->mediaSource) && $this->mediaSource instanceof MODX\Revolution\sources\modMediaSource) {
            return true;
        } else {
            /** @var msProduct $product */
            if ($product = $this->xpdo->getObject('msProduct', ['id' => $this->get('product_id')])) {
                $this->mediaSource = $product->initializeMediaSource();
                if (!$this->mediaSource || !($this->mediaSource instanceof MODX\Revolution\sources\modMediaSource)) {
     ...
     * @return bool|string
     */
    public function generateThumbnails(MODX\Revolution\sources\modMediaSource $mediaSource = null)
    {
    ...
    public function makeThumbnail($options = [], array $info)
    {
        ...
        $this->xpdo->services->add('phpthumb', new modPhpThumb($this->xpdo));

        $phpThumb = $this->xpdo->services->get('phpthumb');
        $phpThumb->initialize();
        ...
        if ($phpThumb->GenerateThumbnail() && $phpThumb->RenderOutput()) {
            $this->xpdo->log(MODX\Revolution\modX::LOG_LEVEL_INFO, '[miniShop2] phpThumb messages for "' . $this->get('url') .
                '". ' . print_r($phpThumb->debugmessages, true));
            $output = $phpThumb->outputImageData;
        } else {
            $this->xpdo->log(MODX\Revolution\modX::LOG_LEVEL_ERROR, '[miniShop2] Could not generate thumbnail for "' .
