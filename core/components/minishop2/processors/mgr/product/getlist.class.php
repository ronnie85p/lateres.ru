...
class msProductGetListProcessor extends GetListProcessor
{
    ...
    public function prepareQueryBeforeCount(xPDO\Om\xPDOQuery $c)
    {
        ...
        if ($this->item_id) {
            ...
            if (!empty($parent)) {
                ...
                $c->query['where'][] = [[
                    new xPDO\Om\xPDOQueryCondition(['sql' => 'msProduct.parent IN ' . $parents, 'conjunction' => 'OR']),
                    new xPDO\Om\xPDOQueryCondition(['sql' => 'Member.category_id IN ' . $parents, 'conjunction' => 'OR'])
                ]];
            }
        }
    ...
    */
    public function prepareQueryAfterCount(xPDO\Om\xPDOQuery $c)
    {
        ...
        $c->query['where'] = [[
            new xPDO\Om\xPDOQueryCondition(['sql' => 'msProduct.id IN ' . $ids, 'conjunction' => 'AND']),
        ]];
