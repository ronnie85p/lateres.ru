...
require_once MODX_CORE_PATH . 'components/pdotools/src/CoreTools.php';
...
class miniShop2
{
    ...
    public function __construct(modX $modx, array $config = [])
    {
        ...
        if (!$this->modx->services->has('pdotools')) {
            $this->modx->services->add('pdotools', new CoreTools($this->modx));    
        }
    ...
    public function changeOrderStatus($order_id, $status_id)
    {
        ...
        if (!$status = $this->modx->getObject('msOrderStatus', ['id' => $status_id, 'active' => 1])) {
        ...
        if ($order->save()) {
            ...
            if ($tmp = $this->modx->getObject('modUserSetting', ['key' => 'cultureKey', 'user' => $order->get('user_id')])) {
                $lang = $tmp->get('value');
            } elseif ($tmp = $this->modx->getObject('modContextSetting', ['key' => 'cultureKey', 'context_key' => $order->get('context')])) {
                $lang = $tmp->get('value');
            }
